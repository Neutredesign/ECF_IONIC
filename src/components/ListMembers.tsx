            ...
            <tr key={member.id}>
              ...
              <td>
                {member.has_voted ? (
                  ...
                  <IonButton className="pink-button" onClick={async () => {
                    try {
                      const res = await fetch(`http://localhost:3000/api/v2/scrutins/${scrutinId}/members/${member.id}/vote`, {
                        method: 'POST',
                      ...
                      setMembers(prev =>
                        prev.map(m => m.id === member.id ? { ...m, has_voted: 1 } : m)
                      );
                    } catch (err) {
