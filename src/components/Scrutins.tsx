...
const Scrutins: React.FC = () => {
  ...
  useEffect(() => {
    const loadScrutins = async () => {
      try {
        const res = await fetch('http://localhost:3000/api/v2/scrutins');
            ...
            <tr key={scrutin.id}>
              ...
              <td>
                <IonButton
                  ...
                  onClick={() => window.location.href = `${scrutin.id}/members/`}
                >
                  Voir
                </IonButton>
              </td>
              <td>
                <IonButton
                  className="pink-button"
                  onClick={() => window.location.href = `${scrutin.id}/members/stats/`}
                >
                  Voir
                </IonButton>
              </td>
